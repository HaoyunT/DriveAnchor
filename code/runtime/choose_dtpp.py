...
def make_fast_choose(reference_choose):
    ...
    def choose(self, xy, route, tracks, ego, speed, lights, lanes):
        if (xy.ndim!=3 or xy.shape[1:]!=(40,2) or not 1<=len(xy)<=500) or not np.isfinite(xy).all():
        ...
        ttc_bad, ttc_diag = self.dtpp_evaluate(xy)
        violations = np.column_stack([collision, outside, red, vbad, abad, ttc_bad])
        index, diagnostic = constrained_choice(-score, violations)
        diagnostic['constraint_names'] = ['predicted_collision', 'lane_footprint_exit', 'current_red_entry', 'speed', 'acceleration', 'dtpp_ttc']
        diagnostic['selected_violations'] = violations[index].tolist()
        diagnostic['constraint_mask_sha256'] = hashlib.sha256(violations.tobytes()).hexdigest()
        diagnostic['rejected_by_constraint'] = violations.sum(0).tolist()
        diagnostic['dtpp_ttc'] = dict(ttc_diag,selected_min_ttc_s=ttc_diag['min_ttc_capped_s'][index],selected_rejected=bool(ttc_bad[index]))
