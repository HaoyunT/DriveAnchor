...
R=pathlib.Path('/autocar/.cache_pt_pytorch2/nuplan_recovery_20260910');out=R/'fm2_top250_cpu_timing_v3';out.mkdir(exist_ok=True)
...
for row in m['scenarios'][:3]:
 ...
 timings={}
 for label,fn in [('original',original_map_process),('cached',obs_adapter.map_process)]:
  values=[]
  for _ in range(12):
   tick=time.perf_counter();fn(*args);values.append((time.perf_counter()-tick)*1000)
  timings[label]={'median_ms':float(np.median(values)),'max_ms':max(values),'values_ms':values}
 print({'token':s.token,'map_tensorization_and_processing':timings},flush=True)
 ...
 checks.append(dict(token=s.token,history=True,filter=True,map_queries=63,features_exact=True,lanes=True,map_processing_timings=timings))
